...
module.exports = class Data1746738904828 {
    ...
    async up(db) {
        ...
        // --- Trade Table and Optimized Indexes ---
        ...
        // OPTIMIZED: Composite index for fetching trades for a token, sorted by time.
        await db.query(`CREATE INDEX "IDX_trade_token_id_timestamp" ON "trade" ("token_id" ASC, "timestamp" DESC)`)
        // OPTIMIZED: Index for fetching a specific user's trades.
        await db.query(`CREATE INDEX "IDX_trade_user" ON "trade" ("user")`)


        // --- Foreign Key Constraints (DISABLED FOR PERFORMANCE) ---
        // Foreign keys add significant overhead to bulk upsert operations.
        // The indexer's application logic ensures data integrity, making these redundant.
    ...
    async down(db) {
        ...
        // These lines would only be needed if the FKs were enabled in `up`.
        ...
        await db.query(`DROP INDEX "public"."IDX_trade_token_id_timestamp"`)
        await db.query(`DROP INDEX "public"."IDX_trade_user"`)
